...
(function($) {
    ...
    var MagicSuggest = function(element, options) {
        ...
        var defaults = {
            ...
            /**
             * If set to true, will search as accen insensitive
             */
        ...
        this.setName = function(name) {
            ...
            if (ms._valueContainer) {
                $.each(ms._valueContainer.children(),
                    function(i, el) {
                        el.name = cfg.name;
                    });
            }
        ...
        var self = {
            ...
            _highlightSuggestion: function (html) {
                ...
                var pattern = {};
                if (cfg.useAccentInsensitive) {
                    pattern = self._createAccentRegexp(q, glob);
                } else {
                    pattern = new RegExp('(' + q + ')(?!([^<]+)?>)', glob);
                }
                return html.replace(pattern, '<em>$1</em>');
            },

            _createAccentRegexp: function(characters, glob) {

                var deaccentedString = self._deaccent(characters);
                var cleanString = deaccentedString.replace(/([|()[{.+*?^$\\])/g, "\\$1");
                var accentReplacer = function(character) {
                    return self._charToAccentedCharClassMap[character] || character;
                };
                var regexp = cleanString.replace(/\S/g, accentReplacer);
                return new RegExp('(' + regexp + ')(?!([^<]+)?>)', glob);
            },

            _deaccent: function(accentedString) {
                var result = accentedString;
                for (var key in self._charToAccentedCharClassMap) {
                    result = result.replace(new RegExp(self._charToAccentedCharClassMap[key], "g"), key);
                }
                return result;
            },

            _charToAccentedCharClassMap: {
                'A':
            ...
            _validateSingleItem: function(value) {
                if (cfg.vregex !== null && cfg.vregex instanceof RegExp) {
                    ...
                    case 'url':
                        return (
                            /(((^https?)|(^ftp)):\/\/([\-\w]+\.)+\w{2,3}(\/[%\-\w]+(\.\w{2,})?)*(([\w\-\.\?\\\/+@&#;`~=%!]*)(\.\w{2,})?)*\/?)/i).test(value);
                    case 'ipaddress':
        ...
        var handlers = {
            ...
            _onKeyUp: function(e) {
                ...
                if ((e.keyCode === KEYCODES.TAB && cfg.useTabKey === false) || (e.keyCode > KEYCODES.ENTER &&
            ...
            _onTriggerClick: function() {
                if (ms.isDisabled() === false && !(cfg.expandOnFocus === true && _selection.length ===
                    cfg.maxSelection)) {
